function [] = GetMusclePolynomials(S,MainPath)
...
% run for Left model
polynomialFit_LMT_dm_3D(ModelPathL, PathLeft,fullfile(DummyMotionPath,'dummy_motion_Left.mot'), max_order, threshold,...
    S.do_muscle_analysis, S.do_polynomial_fitting, if_construct_model);
% run for Right model
polynomialFit_LMT_dm_3D(ModelPathR, PathRight,fullfile(DummyMotionPath,'dummy_motion_Right.mot'), max_order, threshold,...
    S.do_muscle_analysis, S.do_polynomial_fitting, if_construct_model);
