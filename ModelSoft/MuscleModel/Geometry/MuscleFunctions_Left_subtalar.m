function [] = MuscleFunctions_Left_subtalar(S)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

SubjPath    = S.OutPath;
LeftMusclePath = fullfile(SubjPath,'Left');

%% Load all the information
d=load(fullfile(LeftMusclePath,'MuscleInfo_.mat'));
e=load(fullfile(LeftMusclePath,'muscle_spanning_joint_INFO.mat'));
MuscleInfoL=d.MuscleInfo;
muscle_spanning_joint_INFOL=e.muscle_spanning_joint_INFO;
index_q_locked=16;
...
ndof=length(Tracking.output.result.setup.auxdata.NomiCoord_L);
qAll=sym('Q', [1 ndof]);
syms q x0 x1;
ctmat=1;
ctmatdiff=1;
Index_storeMat=nan(3000,2);
Index_storeMatDiff=nan(6000,3);
...
for m=1:length(MuscleInfoL.muscle)
    ...
    index_dof_crossing = find(muscle_spanning_joint_INFOL(muscle_index,:)==1);
    ...
    order=MuscleInfoL.muscle(m).order;
...
f = matlabFunction(mat(:),'File',fullfile(LeftMusclePath,'Get_Mat_Left.m'));
f3= matlabFunction(diff_mat_q(:),'File',fullfile(LeftMusclePath,'Get_Mat_Q_Left.m'));
save(fullfile(LeftMusclePath,'MomentArmInfoL.mat'),'f','f3','Index_storeMatDiff','Index_storeMat');

%% Store all the information in structure
SymbolicInfo=load(fullfile(LeftMusclePath,'MomentArmInfoL.mat'));
MuscleInfoL.SymbolicInfo=SymbolicInfo;
for m=1:length(MuscleInfoL.muscle)
    muscle_index=m;    
    % get the DOF that are spanned by this muscle
    index_dof_crossing = find(muscle_spanning_joint_INFOL(muscle_index,:)==1);
    index_dof_crossing = setdiff(index_dof_crossing, index_q_locked); % Exclude the locked dofs
    nr_dof_crossing=length(index_dof_crossing);    
    MuscleInfoL.muscle(m).index_dof_crossing = index_dof_crossing;
    MuscleInfoL.muscle(m).nr_dof_crossing = nr_dof_crossing;
end

save(fullfile(LeftMusclePath,'MuscleInfoL.mat'),'MuscleInfoL');
