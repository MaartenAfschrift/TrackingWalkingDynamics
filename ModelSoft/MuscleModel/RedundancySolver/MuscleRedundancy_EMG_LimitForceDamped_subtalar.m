function [] = MuscleRedundancy_EMG_LimitForceDamped_subtalar(S,MainPath,MatlabData,OutName,varargin)
...
%% Path info
OutPath     = fullfile(MainPath,'Data',S.SubjName,'SimResults');
OptRes      = load(fullfile(OutPath,OutName));
OptRes.info.CARTELLAOUTPUT=OutPath;
EMGdat_all  = load(fullfile(MatlabData,'MeanData','EMG','EMG_Increase_norm_detail_NPhase.mat'));
MuscleInfoPathLeft  = fullfile(MainPath,'Data',S.SubjName,'Left_subtalar');
MuscleInfoPathRight = fullfile(MainPath,'Data',S.SubjName,'Right_subtalar');

S.KsName_Left   =fullfile(OutPath,'KS_Left_TrackinSim_subtalar.mot');
S.KsName_Right  =fullfile(OutPath,'KS_Right_TrackinSim_subtalar.mot');
S.IdName_Left   =fullfile(OutPath,'ID_Left_TrackingSim_subtalar.sto');
S.IdName_Right  =fullfile(OutPath,'ID_Right_TrackingSim_subtalar.sto');

%% Add paths to matlab
addpath(genpath(fullfile(MainPath,'Software','SolveMuscleRedundancy')));
addpath(genpath(fullfile(MainPath,'Software','MexFiles')));
addpath(genpath(fullfile(MainPath,'Software','Functions')));
...
if S.MuscleFunctionsGeneric == 0
    ...
    MuscleInfoPathLeft  = fullfile(MainPath,'Data','Patient2','Left_subtalar');
    MuscleInfoPathRight = fullfile(MainPath,'Data','Patient2','Right_subtalar');
    addpath(genpath(MuscleInfoPathLeft));      % functions to compute moment arms
    addpath(genpath(MuscleInfoPathRight));     % functions to compute moment arms
end
...
%% Add EMG constraints
GML_ref=EMGdat_all.EMG_RefStore(S.SubjNumber,:,8);
GMR_ref=EMGdat_all.EMG_RefStore(S.SubjNumber,:,16);
...
MuscleAnalysisPathL=fullfile(OutPath,'MuscleAnalysisModelLeft_subtalar'); if ~isdir(MuscleAnalysisPathL); mkdir(MuscleAnalysisPathL); end;
if S.if_RunMuscleAnalaysis; OpenSim_Muscle_Analysis(S.KsName_Left,LmodelPath,MuscleAnalysisPathL,[time(1) time(end)]); end;
MuscleAnalysisPathR=fullfile(OutPath,'MuscleAnalysisModelRight_subtalar'); if ~isdir(MuscleAnalysisPathR); mkdir(MuscleAnalysisPathR); end;
...
if S.LeftModel
    ...
    %     save(fullfile(OutPath,'Results_MuscleRedundancyLeftModel_vA_EMG.mat'),'output','DatStore','Misc','OptRes');
    ...
    %     plot(time,GML_int.*output.result.solution.parameter,'--r','LineWidth',2);
    %     subplot(1,2,2);
    %     plot(output.result.solution.phase.control(:,31:35)*150);
    clear DatStore Misc
%     if S.RightModel
%         delete(fullfile(pwd,'musdynContinous_FtildeState_GM_dampedADiGatorGrd.m'));
%         delete(fullfile(pwd,'musdynContinous_FtildeState_GM_dampedADiGatorGrd.mat'));
%         delete(fullfile(pwd,'musdynContinous_FtildeState_GM_dampedADiGatorHes.m'));
%         delete(fullfile(pwd,'musdynContinous_FtildeState_GM_dampedADiGatorHes.mat'));
%         delete(fullfile(pwd,'musdynEndpoint_FtildeStateADiGatorGrd.m'));
%         delete(fullfile(pwd,'musdynEndpoint_FtildeStateADiGatorGrd.mat'));
%         delete(fullfile(pwd,'musdynEndpoint_FtildeStateADiGatorHes.m'));
%         delete(fullfile(pwd,'musdynEndpoint_FtildeStateADiGatorHes.mat'));
%     end
end
