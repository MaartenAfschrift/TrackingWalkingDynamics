function OUT=DoubleModel_AD_CONTINUOUS_mpi_Contact(IN)
global BoolFirst
...
% get copy of the static parameters (contact model properties
Stiffness   =IN.phase.parameter(1);
% Yoffset     =IN.phase.parameter(2); 
...
[F_L,M_L]=POSVELaar2GRF_GPOPS_Contact2_old(PV_PT_L,IN.auxdata.ContactInfo,Stiffness,u,c,vt);
M_L=-M_L;
% R
PV_PT_R=contactPointsQeU_GPOPS_Vect_Contact(Q_R(:,1:3),Q_R(:,4:6),U_R(:,1:3),U_R(:,4:6),IN.auxdata.ContactInfo);
[F_R,M_R]=POSVELaar2GRF_GPOPS_Contact2_old(PV_PT_R,IN.auxdata.ContactInfo,Stiffness,u,c,vt);
M_R=-M_R;
...
deltaTrackTRC(:,2:3:end)=deltaTrackTRC(:,2:3:end)*0.1;     % less weight on tracking vertical position
...
if SCALA
    ...
    OUT.path=[deltaF_L deltaM_L deltaF_R deltaM_R deltaPT];
end
