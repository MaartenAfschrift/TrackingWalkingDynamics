...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	TCHAR szNamePlatformAcc[] = TEXT("Global\\PlatformAcc");
	...
	else
	{
		...
		double* PlatformAcc = mxGetPr(prhs[15]);
		...
		HANDLE hMapFile_PlatformAcc;		// platform acceleration( perturbation)
		...
		hMapFile_PlatformAcc = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, BUF_SIZE_TIME, szNamePlatformAcc);
	   ...
	   //}
		...
		pBuf_PlatformAcc = (double*)MapViewOfFile(hMapFile_PlatformAcc, FILE_MAP_ALL_ACCESS, 0, 0, BUF_SIZE_TIME);
		...
		for (int i = 0; i < numPts; ++i) // for all rows of data i
		{
			...
			pBuf_PlatformAcc[i] = PlatformAcc[i];
		}
		...
		*pBuf_ResReady = false;
		...
		UnmapViewOfFile(pBuf_PlatformAcc);
		...
		CloseHandle(hMapFile_PlatformAcc);
	}
}
//-End MEX enterance ------------------------------------------------------
