function [output] = TrackingSim_ID_ImprovedContact(S,MPI_path,OutName,varargin)
...
[ContactProp]=AddContactModel(S,OutPath);
...
[OUT_L]=DoubleModelInitailGuess(LeftModelName,[OutPath,'\INVERSEMODELKINEMATICS_L.mot'],GRFinput);
...
SwitchModels();
pause(1);       % pause 1 second to make sure that models are loaded before starting optimization (not sure if this is needed)
if S.ifPause
    pause();
end

output=runFirstOptimization_AccContr_subtalar_locked_GRFadj('NMeshIntervals',40,'ScalingBound',1.5,...
    'NColPoints',4,'LeftModelName',LeftModelName,'scala',false,...
    'RightModelName',RightModelName,'GRFdata',GRDdataXopt,...
    'Time',T,'MarkerInfo',MArkerInfoXopt,'OUTtuneGRF',ContactProp,...
