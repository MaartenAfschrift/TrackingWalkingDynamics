function [varargout]=runFirstOptimization_AccContr_subtalar_locked_GRFadj(varargin)
...
IND_coord_NO_L=strcmpi(NomiCoord_L,'mtp_angle_l') + strcmpi(NomiCoord_L,'subtalar_angle_l') ;
IND_coord_OK_L=~IND_coord_NO_L;
IND_coord_OK_L=find(IND_coord_OK_L);
IND_coord_NO_L=find(IND_coord_NO_L);
IND_coord_NO_R=strcmpi(NomiCoord_R,'mtp_angle_r')+ strcmpi(NomiCoord_R,'subtalar_angle_r');
...
% Rate=1./nanmean(diff(T)); Rate=1./nanmean(diff(T));
Rate=1./mean(diff(T));
Frames=1:length(T);
Time=T;
Units='m';
